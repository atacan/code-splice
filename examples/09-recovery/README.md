...
```bash
srcmv --workspace examples/.work/09-recovery/workspace recover --list
srcmv --workspace examples/.work/09-recovery/workspace recover --list --json
...
```bash
srcmv --workspace /path/to/workspace recover --list --json
srcmv --workspace /path/to/workspace recover TRANSACTION_ID --status --json
```
...
```bash
srcmv --workspace /path/to/workspace recover TRANSACTION_ID --complete --json
srcmv --workspace /path/to/workspace recover TRANSACTION_ID --rollback --json
```
...
demonstration lives in `crates/srcmv-cli/tests/codex_pilot.rs` and is run by
